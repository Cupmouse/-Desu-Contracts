...
contract('DesuBoard', async (accounts) => {
  ...
  describe('List', async () => {
    ...
    const callDetachThread = async (index, accountAs) => {
      await desuBoard.lock({from: accountAs});
      await desuBoard.detachThreadByIndex(index, {from: accountAs});
      await desuBoard.unlock({from: accountAs});
    ...
    const callGetSize = async () => await desuBoard.getNumberOfThreads.call({from: accounts[1]});
